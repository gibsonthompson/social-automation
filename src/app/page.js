...
export default function ContentFarm() {
  ...
  useEffect(() => { if (ready) lsSet('cf_businesses', businesses); }, [businesses, ready]);
  useEffect(() => {
    if (ready) {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
          ...
          {navItems.map((n) => {
            ...
            return (
              <button key={n.id} onClick={() => setPage(n.id)} style={{
                display: 'flex', alignItems: 'center', gap: 9, width: '100%', padding: '9px 10px', border: 'none',
                background: active ? 'var(--s1)' : 'transparent', color: active ? 'var(--tx)' : 'var(--tx-muted)',
                borderRadius: 7, cursor: 'pointer', fontSize: 13, fontWeight: active ? 600 : 400, fontFamily: 'inherit', marginBottom: 1,
              }}>
                <span style={{ opacity: active ? 1 : 0.4 }}><Icon name={n.icon} size={16} /></span>{n.label}
              </button>
            );
          })}
        </nav>
      ...
      <div style={{ flex: 1, overflow: 'auto', background: 'var(--bg)' }}>
        {page === 'generate' && <GeneratePage businesses={businesses} photos={photos} addToLibrary={addToLibrary} />}
        {page === 'businesses' && <BusinessesPage businesses={businesses} setBusinesses={setBusinesses} />}
        {page === 'photos' && <PhotoBankPage businesses={businesses} photos={photos} setPhotos={setPhotos} />}
        {page === 'library' && <LibraryPage library={library} businesses={businesses} setLibrary={setLibrary} />}
      </div>
    </div>
  );
}
...
function GeneratePage({ businesses, photos, addToLibrary }) {
  ...
  // Refresh feedback stats when business changes
  useEffect(() => {
  ...
  const generateBatch = async () => {
    ...
    try {
      const feedbackItems = getFeedbackForAPI(bizId);

      const resp = await fetch('/api/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ business: biz, mode: 'batch', feedback: feedbackItems }),
      });
  ...
  const renderAllImages = async (items, bizData) => {
    ...
    for (let i = 0; i < updated.length; i++) {
      ...
      let photo = null;
      if ((item.result.template === 'photo_feature' || item.result.template === 'service_spotlight') && bizPhotos.length > 0) {
  ...
  const toggleSelect = (idx) => setBatch((prev) => prev.map((item, i) => i === idx ? { ...item, selected: !item.selected } : item));
  ...
  const downloadAllZip = async () => {
    ...
    selected.forEach((item, idx) => {
      ...
      zip.file(`${biz?.slug || 'post'}-${idx + 1}-${item.result?.content_type || 'post'}.png`, base64, { base64: true });
    });
    const blob = await zip.generateAsync({ type: 'blob' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `${biz?.slug || 'content'}-batch-${Date.now()}.zip`;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
  ...
  const copyAllCaptions = () => {
    ...
    if (selected.length === 0) return;
    const text = selected.map((item, idx) => {
      const r = item.result;
  ...
  const saveSelectedToLibrary = () => {
    ...
    const items = selected.map((item) => ({
      ...
      biz_id: bizId, biz_name: biz?.name || '', tpl: item.result.template,
      content: item.result, image_data: item.imageData, created: new Date().toISOString(),
    }));
    addToLibrary(items);
  };

  // ── Feedback handlers ──────────────────────────────────────────
  const handleFeedback = (idx, rating, reason) => {
    ...
    setBatch((prev) => prev.map((it, i) => i === idx ? { ...it, feedbackGiven: true, feedbackRating: rating } : it));
    setFbStats(getFeedbackStats(bizId));
  };

  const handleClearFeedback = () => {
    clearFeedback(bizId);
    setFbStats({ total: 0, good: 0, bad: 0 });
  };
    ...
    <div style={{ padding: 28 }}>
      ...
      <div style={{ marginBottom: 22 }}>
        ...
        <p style={{ color: 'var(--tx-muted)', fontSize: 13, marginTop: 4 }}>
          12 unique posts per batch. Rate each post to train the AI on what works for each business.
        </p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 14, alignItems: 'flex-end', marginBottom: 22, flexWrap: 'wrap' }}>
        <div style={{ minWidth: 220 }}>
          <Select label="Business" value={bizId} onChange={(v) => { setBizId(v); setBatch([]); }}
        ...
        {fbStats.total > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: 'var(--s1)', borderRadius: 8, border: '1px solid var(--bd)' }}>
            ...
            <button onClick={handleClearFeedback} style={{
              background: 'none', border: 'none', color: 'var(--tx-dim)', fontSize: 10, cursor: 'pointer', fontFamily: 'inherit', textDecoration: 'underline',
            }}>clear</button>
          </div>
        )}
      </div>
      ...
      {error && (
        <div style={{ padding: '12px 18px', background: 'rgba(231,74,74,0.08)', border: '1px solid rgba(231,74,74,0.2)', borderRadius: 10, color: 'var(--red)', fontSize: 13, marginBottom: 20 }}>
          {error}
        </div>
      )}
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: '60px 20px' }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--tx-muted)', fontWeight: 500 }}>Generating 12 posts for {biz?.name}...</div>
          <div style={{ fontSize: 12, color: 'var(--tx-dim)', marginTop: 6 }}>12 parallel AI calls. ~10-15 seconds.{fbStats.total > 0 ? ` Learning from ${fbStats.total} past ratings.` : ''}</div>
        </div>
      )}
      ...
      {rendering && !loading && (
        <div style={{ padding: '12px 18px', background: 'rgba(201,164,76,0.08)', border: '1px solid rgba(201,164,76,0.2)', borderRadius: 10, color: 'var(--gold)', fontSize: 13, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 16, height: 16, border: '2px solid var(--bd)', borderTop: '2px solid var(--gold)', borderRadius: '50%', animation: 'spin .7s linear infinite' }} />
          Rendering images...
        </div>
      )}
        ...
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              {batch.some((item) => !item.success) && <Tag color="var(--red)">{12 - successCount} failed</Tag>}
              <span style={{ fontSize: 12, color: 'var(--tx-dim)' }}>{selectedCount} selected</span>
            </div>
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              <Btn size="sm" onClick={copyAllCaptions} disabled={selectedCount === 0}>
                <Icon name={allCopied ? 'check' : 'copy'} size={12} /> {allCopied ? 'Copied' : 'Copy Captions'}
              </Btn>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
            {batch.map((item, idx) => (
              <BatchCard key={item.id || idx} item={item} idx={idx}
                onToggle={() => toggleSelect(idx)}
                onDownload={() => downloadOne(item, idx)}
                onFeedback={(rating, reason) => handleFeedback(idx, rating, reason)} />
            ))}
          </div>
        </>
      )}
      ...
      {batch.length === 0 && !loading && (
        <div style={{ textAlign: 'center', padding: '60px 20px' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--tx-dim)', marginTop: 5 }}>
            12 unique posts with varied templates and content angles.
            {fbStats.total > 0 ? ` AI has learned from ${fbStats.total} ratings for this business.` : ' Rate posts to teach the AI what works.'}
          </div>
        </div>
      )}
    </div>
...
// ══════════════════════════════════════════════════════════════════════
// BATCH CARD WITH FEEDBACK
// ══════════════════════════════════════════════════════════════════════
function BatchCard({ item, idx, onToggle, onDownload, onFeedback }) {
  const [expanded, setExpanded] = useState(false);
  const [captionCopied, setCaptionCopied] = useState(false);
  const [feedbackMode, setFeedbackMode] = useState(null); // null | 'good' | 'bad'
  const [feedbackText, setFeedbackText] = useState('');

  if (!item.success) {
    return (
      <div style={{ background: 'var(--s1)', border: '1px solid rgba(231,74,74,0.2)', borderRadius: 12, padding: 20, textAlign: 'center' }}>
        <div style={{ fontSize: 13, color: 'var(--red)', fontWeight: 600, marginBottom: 4 }}>Post {idx + 1} failed</div>
        <div style={{ fontSize: 11, color: 'var(--tx-dim)' }}>{item.error || 'Unknown error'}</div>
        <div style={{ marginTop: 8 }}><Tag color="var(--tx-dim)">{item.planItem?.category || 'unknown'}</Tag></div>
      </div>
    );
  }
  ...
  const r = item.result;
  const tplColors = {
    bold_statement: 'var(--gold)', photo_feature: 'var(--blue)', tip_card: 'var(--green)',
    stat_callout: 'var(--purple)', service_spotlight: 'var(--red)',
  };

  const copyCaption = () => {
    const full = (r.caption || '') + '\n\n' + (r.hashtags || []).map((h) => '#' + h).join(' ');
    navigator.clipboard.writeText(full).then(() => { setCaptionCopied(true); setTimeout(() => setCaptionCopied(false), 2000); });
  };

  const submitFeedback = (rating) => {
    onFeedback(rating, feedbackText.trim());
  ...
  return (
    <div style={{
      background: 'var(--s1)', border: `1px solid ${item.selected ? 'var(--gold)' : 'var(--bd)'}`,
      borderRadius: 12, overflow: 'hidden', transition: 'border-color 0.15s', opacity: item.selected ? 1 : 0.55,
    }}>
      {/* Image */}
      {item.imageData ? (
        <div onClick={onToggle} style={{
          width: '100%', aspectRatio: '1080/1350', backgroundImage: `url(${item.imageData})`,
          backgroundSize: 'cover', backgroundPosition: 'center', cursor: 'pointer', position: 'relative',
        }}>
          <div style={{
          ...
          {item.feedbackGiven && (
            <div style={{
              ...
              color: item.feedbackRating === 'good' ? 'var(--green)' : 'var(--red)',
            }}>
              {item.feedbackRating === 'good' ? 'APPROVED' : 'REJECTED'}
            </div>
          )}
        </div>
      ) : (
        <div onClick={onToggle} style={{
          width: '100%', aspectRatio: '1080/1350', background: 'var(--s2)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          cursor: 'pointer', fontSize: 12, color: 'var(--tx-dim)',
        }}>Rendering...</div>
      )}
      ...
      <div style={{ padding: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
          <div style={{ display: 'flex', gap: 4 }}>
            <Tag color={tplColors[r.template] || 'var(--tx-dim)'}>{r.template?.replace('_', ' ')}</Tag>
        ...
        {expanded && (
          <div style={{
            fontSize: 11, lineHeight: 1.6, whiteSpace: 'pre-wrap', marginBottom: 8,
            padding: '8px 10px', background: 'var(--bg)', borderRadius: 6, border: '1px solid var(--bd)',
          }}>
            {r.caption}
            <div style={{ marginTop: 6, display: 'flex', flexWrap: 'wrap', gap: 3 }}>
              {(r.hashtags || []).map((h, i) => <Tag key={i} color="var(--purple)">#{h}</Tag>)}
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: item.feedbackGiven ? 0 : 8 }}>
          <Btn size="sm" variant="ghost" onClick={() => setExpanded(!expanded)}>{expanded ? 'Less' : 'More'}</Btn>
          <Btn size="sm" variant="ghost" onClick={copyCaption}><Icon name={captionCopied ? 'check' : 'copy'} size={11} /></Btn>
          ...
          <div style={{ borderTop: '1px solid var(--bd)', paddingTop: 8, marginTop: 4 }}>
            {feedbackMode === null ? (
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                ...
                <button onClick={() => { submitFeedback('good'); }} style={{
                  background: 'rgba(52,199,123,0.1)', border: '1px solid rgba(52,199,123,0.2)',
                  borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: 11, fontWeight: 600,
                  color: 'var(--green)', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 4,
                }}>Good</button>
                <button onClick={() => setFeedbackMode('bad')} style={{
                  background: 'rgba(231,74,74,0.1)', border: '1px solid rgba(231,74,74,0.2)',
                  borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: 11, fontWeight: 600,
                  color: 'var(--red)', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 4,
                }}>Bad</button>
                <button onClick={() => setFeedbackMode('good')} style={{
                  background: 'none', border: 'none', color: 'var(--tx-dim)', fontSize: 10,
                  cursor: 'pointer', fontFamily: 'inherit', textDecoration: 'underline',
                }}>+ note</button>
              </div>
            ) : (
              <div>
                <div style={{ fontSize: 10, color: feedbackMode === 'good' ? 'var(--green)' : 'var(--red)', fontWeight: 700, marginBottom: 4, textTransform: 'uppercase' }}>
                  {feedbackMode === 'good' ? 'What did you like?' : 'What was wrong?'}
                </div>
                <textarea
                  value={feedbackText}
                  onChange={(e) => setFeedbackText(e.target.value)}
                  placeholder={feedbackMode === 'good' ? 'Great tone, keep this angle...' : 'Too generic, needs more local specificity...'}
                  style={{
                    width: '100%', background: 'var(--bg)', border: '1px solid var(--bd)', borderRadius: 6,
                    padding: '6px 8px', color: 'var(--tx)', fontSize: 11, fontFamily: 'inherit',
                    resize: 'vertical', minHeight: 50, outline: 'none', boxSizing: 'border-box',
                  }}
                />
                <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
                  <button onClick={() => submitFeedback(feedbackMode)} style={{
                    background: feedbackMode === 'good' ? 'var(--green)' : 'var(--red)',
                    border: 'none', borderRadius: 5, padding: '4px 12px', cursor: 'pointer',
                    fontSize: 11, fontWeight: 600, color: '#fff', fontFamily: 'inherit',
                  }}>Submit</button>
                  <button onClick={() => { setFeedbackMode(null); setFeedbackText(''); }} style={{
                    background: 'none', border: 'none', color: 'var(--tx-dim)', fontSize: 11,
...
function BusinessesPage({ businesses, setBusinesses }) {
  ...
  const startAdd = () => {
    setForm({
      id: 'biz_' + Date.now(), name: '', slug: '', website: '', industry: 'consulting',
      industry_label: '', tagline: '', primary_color: '#3B82F6', secondary_color: '#60A5FA',
      accent_color: '#F59E0B', bg_color: '#0A0A14', text_color: '#FFFFFF', tone: '', icp: '',
      services: '', service_areas: '', certifications: '', cta_phrases: '', fact_sheet: '', banned_words: '',
    });
    setEditing('new');
  };
  const save = () => {
    if (!form.name) return;
    if (editing === 'new') {
      form.slug = form.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 20);
  ...
  const industryOptions = [
    ...
    { value: 'logistics_advisory', label: 'Logistics Advisory' },
  ];
    ...
    <div style={{ padding: 28 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22 }}>
        <div>
          ...
          <p style={{ color: 'var(--tx-muted)', fontSize: 13, marginTop: 4 }}>Each business gets unique prompt strategies based on its industry type.</p>
        </div>
        ...
        {businesses.map((b) => (
          <div key={b.id} style={{ background: 'var(--s1)', border: '1px solid var(--bd)', borderRadius: 12, overflow: 'hidden' }}>
            ...
            <div style={{ padding: 16 }}>
              ...
              {b.tone && <div style={{ fontSize: 11, color: 'var(--tx-muted)', lineHeight: 1.4, marginBottom: 10, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.tone}</div>}
      ...
      {editing !== null && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.72)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
          ...
          <div style={{ background: 'var(--bg)', border: '1px solid var(--bd)', borderRadius: 14, width: '92%', maxWidth: 660, maxHeight: '88vh', overflow: 'auto', padding: 26 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ fontSize: 18, fontWeight: 700 }}>{editing === 'new' ? 'Add Business' : 'Edit Business'}</h2>
...
function PhotoBankPage({ businesses, photos, setPhotos }) {
  ...
  const upload = async (e) => {
    ...
    setPhotos((prev) => ({ ...prev, [bizId]: [...(prev[bizId] || []), ...arr] }));
    setBusy(false);
    if (fileRef.current) fileRef.current.value = '';
  };

  const deletePhoto = (idx) => setPhotos((prev) => ({ ...prev, [bizId]: (prev[bizId] || []).filter((_, i) => i !== idx) }));

  const updateTag = (idx, category, mood) => {
    setPhotos((prev) => {
      const updated = [...(prev[bizId] || [])];
      if (updated[idx]) updated[idx] = { ...updated[idx], category, mood };
  ...
  return (
    <div style={{ padding: 28 }}>
      <div style={{ marginBottom: 22 }}>
        ...
        <p style={{ color: 'var(--tx-muted)', fontSize: 13, marginTop: 4 }}>Upload and tag photos per business. Photos stay in memory for this session.</p>
      </div>
      <div style={{ display: 'flex', gap: 14, alignItems: 'flex-end', marginBottom: 22 }}>
        <div style={{ minWidth: 220 }}>
          <Select label="Business" value={bizId} onChange={setBizId} options={businesses.map((b) => ({ value: b.id, label: b.name }))} />
        </div>
      ...
      {bizPhotos.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '50px 20px', background: 'var(--s1)', borderRadius: 12, border: '1px dashed var(--bd)' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--tx-dim)', marginTop: 4 }}>Upload images the AI can composite into generated posts.</div>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(190px, 1fr))', gap: 12 }}>
          {bizPhotos.map((photo, idx) => (
            <div key={idx} style={{ background: 'var(--s1)', border: '1px solid var(--bd)', borderRadius: 10, overflow: 'hidden' }}>
              <div style={{ width: '100%', aspectRatio: '4/3', backgroundImage: `url(${photo.data})`, backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative' }}>
                <button onClick={() => deletePhoto(idx)} style={{ position: 'absolute', top: 5, right: 5, background: 'rgba(0,0,0,.7)', border: 'none', color: 'var(--red)', cursor: 'pointer', borderRadius: 5, padding: '3px 5px', display: 'flex' }}>
                  <Icon name="trash" size={12} />
                </button>
              </div>
              <div style={{ padding: 8 }}>
                <div style={{ fontSize: 10, color: 'var(--tx-dim)', marginBottom: 5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{photo.filename}</div>
                <div style={{ display: 'flex', gap: 4 }}>
                  <select value={photo.category} onChange={(e) => updateTag(idx, e.target.value, photo.mood)} style={{ flex: 1, background: 'var(--bg)', border: '1px solid var(--bd)', borderRadius: 5, padding: '3px 5px', color: 'var(--tx)', fontSize: 10, fontFamily: 'inherit' }}>
                    {categories.map((c) => <option key={c} value={c}>{c}</option>)}
                  </select>
                  <select value={photo.mood} onChange={(e) => updateTag(idx, photo.category, e.target.value)} style={{ flex: 1, background: 'var(--bg)', border: '1px solid var(--bd)', borderRadius: 5, padding: '3px 5px', color: 'var(--tx)', fontSize: 10, fontFamily: 'inherit' }}>
...
function LibraryPage({ library, businesses, setLibrary }) {
  ...
  const downloadItem = (item) => {
    ...
    const a = document.createElement('a');
    a.href = item.image_data; a.download = `${item.biz_id}-${item.id}.png`;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
  };

  const copyCaption = (item) => {
    const text = (item.content?.caption || '') + '\n\n' + (item.content?.hashtags || []).map((h) => '#' + h).join(' ');
    navigator.clipboard.writeText(text).then(() => { setCopiedId(item.id); setTimeout(() => setCopiedId(null), 2000); });
  };

  const deleteItem = (id) => setLibrary((prev) => prev.filter((x) => x.id !== id));

  const filterOptions = [{ value: 'all', label: 'All' }, ...businesses.map((b) => ({ value: b.id, label: b.name.split(' ').map((w) => w[0]).join('').slice(0, 3) }))];

  return (
    <div style={{ padding: 28 }}>
      <div style={{ marginBottom: 22 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700 }}>Content Library</h1>
        <p style={{ color: 'var(--tx-muted)', fontSize: 13, marginTop: 4 }}>{library.length} items. Image data only persists for the current session.</p>
      </div>
      <div style={{ display: 'flex', gap: 4, marginBottom: 18, flexWrap: 'wrap' }}>
        {filterOptions.map((f) => (
          <button key={f.value} onClick={() => setFilter(f.value)} style={{
            padding: '5px 13px', borderRadius: 6, border: `1px solid ${filter === f.value ? 'var(--gold)' : 'var(--bd)'}`,
            background: filter === f.value ? 'rgba(201,164,76,0.08)' : 'var(--s1)', color: filter === f.value ? 'var(--gold)' : 'var(--tx-muted)',
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '50px 20px', background: 'var(--s1)', borderRadius: 12, border: '1px dashed var(--bd)' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--tx-dim)', marginTop: 4 }}>Generate content and save it here.</div>
        </div>
      ) : (
            ...
            <div key={item.id} style={{ background: 'var(--s1)', border: '1px solid var(--bd)', borderRadius: 12, overflow: 'hidden' }}>
              ...
              <div style={{ padding: 14 }}>
                ...
                <div style={{ display: 'flex', gap: 4, marginTop: 10 }}>
                  <Btn size="sm" onClick={() => downloadItem(item)}><Icon name="download" size={12} /></Btn>
                  <Btn size="sm" onClick={() => copyCaption(item)}><Icon name={copiedId === item.id ? 'check' : 'copy'} size={12} /></Btn>
                  <Btn size="sm" variant="danger" onClick={() => deleteItem(item.id)}><Icon name="trash" size={12} /></Btn>
                </div>
              </div>
            </div>
          ))}
